...
public class HandController : MonoBehaviour
{
    ...
    // private const float THROW_VELOCITY_COEFFICIENT = 0.125f;
    ...
    public void Release()
    {
        ...
        // Method #2: Function
        float throwVelocityCoeff = Mathf.Clamp(0.125f / Vector3.Distance(grabbingTransform.position, this.transform.position), 0.05f, 1.0f);
        grabbingTransform.GetComponent<Rigidbody>().velocity = throwVelocityCoeff * (grabbingTransform.position - savedPos[grabbingTransformSavedCount - 1]) / (Time.time - savedTimes[grabbingTransformSavedCount - 1]); // velocity calculation using nth most recent frame and current frame, to prevent influence of potential flicking in the last frames
